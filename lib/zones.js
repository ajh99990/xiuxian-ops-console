...
function zoneUnavailableReason(error) {
  const message = formatError(error);

  if (/你已探索过此福地\/凶地|已探索过此福地\/凶地|需等待下次刷新|已探索过.*(?:福地|凶地)/.test(message)) {
    return '已探索过，需等待下次刷新';
  }

  if (isZoneTakenError(error)) return '机缘已被他人先一步夺取';

  return '';
}

function isZoneUnavailableError(error) {
  return Boolean(zoneUnavailableReason(error));
}
